...
const Bosisler = ({ users, posts, categories, news }) => {
  const { Meta } = Card;
  ...
  const filterUsersById = (id) => {
    const filtered = users && users.data.filter((user) => user._id === id);
    console.log(filtered);
    return filtered && filtered.length > 0 ? filtered[0].username : null;
  };
